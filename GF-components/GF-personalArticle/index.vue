	...
	<div>
		<view class="dynamic">
			<view class="avatar">
				...
				<u-tag :text="status" :type="statusType" class="button" icon='hourglass'></u-tag>
			</view>
			<view class="content">
			...
			<view class="option">
				<u-row justify="space-between" align='center' gutter='10'>
					...
					<u-col span="2" style='margin-left:3px;'>
						<u-icon name="share" size="25" @click='show=true'></u-icon>
					</u-col>
				</u-row>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			judgeStatus() {
				if (this.item.status === 'Reviewing') {
					this.status = '审核中'
					this.statusType = 'warning';
				} else if (this.item.status === 'Failed') {
					this.status = '审核未通过'
